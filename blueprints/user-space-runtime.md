...
- A launcher `activity-alias` is routing metadata, not a loadable guest class; imports persist its normalized target Activity, and slot preparation resolves legacy alias metadata from the verified snapshot before class loading.
...
- [x] Launcher aliases resolve to their target Activity implementation for new imports and legacy instance metadata.
...
- Launcher-class and alias-target resolution plus host stub routing are implemented, but guest resources, class initialization, `Application`/Activity attachment, PackageManager and ActivityManager virtualization, and native-library loading are not.
